...
abstract class GenericParams
{
    ...
    public static function __getUnivYear($adapter=NULL){
        $session = new Container(self::__NAME_CONTAINER__);
        $year=$session->offsetGet(self::__NAME_YEAR__);
        if(empty($year)){
            if($adapter){
                $anneunivTable = new \Application\Model\Table\AnneescolaireTable($adapter);

                $anneeRecords=$anneunivTable->select("anneeactive=1");
